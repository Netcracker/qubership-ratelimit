...
**What is built today**: the two resources, their validation, and the lifecycle around them — atomic generations,
last-good fallback, and the transaction gate on a mapping update. The rules are compiled and validated on every event;
what is not built is the evaluation of them. The RLS endpoint answers from a stub that allows one request per second per
claimed domain, so a deployment enforces that and not the rules of the policy. Everything below about counters, windows
and algorithms is the contract the schema expresses and the compiler checks, not yet what the endpoint does.
