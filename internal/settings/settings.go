// Package settings reads the properties of the data plane that the one binary
// and the service interpret the same way: where the counters live, the
// near-limit margin of the metrics, and the claims and roles the management
// API authorizes against. Each function reads configloader and returns a
// value the wiring uses; a bad value is logged and replaced by the default,
// never fatal, because none of these is worth a pod that does not start.
package settings
