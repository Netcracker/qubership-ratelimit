...
type Updater struct {
	...
	// engines are the engines of the previous rebuild, reused for domains whose
...
// unchanged rules — and a reused engine keeps its warm token cache. The cache
// holds extraction results, themselves a pure function of the token and the
// snapshot's extraction plan, so nothing stale survives the reuse; a domain
// whose rules did change gets a fresh engine, and the old cache retires with
// the old one.
func (u *Updater) ruleSet(result *policy.Result, previous map[string]policy.Bundle) *RuleSet {
	...
	for domain, snapshot := range result.Snapshots {
		if prev, ok := u.engines[domain]; ok && unchanged(previous[domain], result.State[domain]) {
			engines[domain] = prev
			continue
		}
