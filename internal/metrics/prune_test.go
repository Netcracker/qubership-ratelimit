...
func pruneTestSet() *ActiveSet {
	return &ActiveSet{
		...
		Rules:   map[string]struct{}{"p/b/kept": {}},
...
func TestPruneStale_dropsTheSeriesOfRenamedObjects(t *testing.T) {
	...
	keptBefore := testutil.ToFloat64(Decisions.WithLabelValues("prune.alive", "p/b/kept", OutcomeOK))
	Decisions.WithLabelValues("prune.alive", "p/b/kept", OutcomeOK).Inc()
	Decisions.WithLabelValues("prune.alive", "p/b/renamed", OutcomeOK).Inc()
	...
	assert.Equal(t, keptBefore+1, testutil.ToFloat64(Decisions.WithLabelValues("prune.alive", "p/b/kept", OutcomeOK)),
		"a live series keeps its value")
	assert.Zero(t, testutil.ToFloat64(Decisions.WithLabelValues("prune.alive", "p/b/renamed", OutcomeOK)),
...
func TestPruneStale_aRepublishedSetRevivesTheJudgment(t *testing.T) {
	...
	Decisions.WithLabelValues("prune.alive", "p/b/comeback", OutcomeOK).Inc()
	PruneStale(pruneTestSet())

	restored := pruneTestSet()
	restored.Rules["p/b/comeback"] = struct{}{}
	PruneStale(restored)
	Decisions.WithLabelValues("prune.alive", "p/b/comeback", OutcomeOK).Inc()

	pruneOnce()
	assert.Equal(t, 1.0, testutil.ToFloat64(Decisions.WithLabelValues("prune.alive", "p/b/comeback", OutcomeOK)),
...
func TestPruneStale_isSafeUnderConcurrentPublication(t *testing.T) {
	...
	churn := &ActiveSet{
		...
		Rules:   map[string]struct{}{"p/b/churn": {}},
	...
	for range 4 {
		wg.Go(func() {
			for range 25 {
				PruneStale(churn)
				Decisions.WithLabelValues("prune.churn", "p/b/churn", OutcomeOK).Inc()
				pruneOnce()
			}
		})
	}
