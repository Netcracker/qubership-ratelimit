...
func TestApplicability_clientAloneLeavesThePlanUndecided(t *testing.T) {
	...
	everyone := rules["cascade/everyone"]
	...
	require.Equal(t, "cascade/premium", everyone.ConditionalOn[0].Rule)
}
...
func TestApplicability_decidingThePlanDecidesTheCascade(t *testing.T) {
	...
	require.Equal(t, ruleview.ApplicabilityNever, rules["cascade/everyone"].Applicability)
}
...
func TestApplicability_replacesPreemptsInsideAnAllBlock(t *testing.T) {
	...
	perClient := rules["orders/per-client"]
...
func TestApplicability_gatesAreMinimal(t *testing.T) {
	...
	block := findBlock(t, snapshot, "cascade")
	view := ruleview.Block(block)
	annotate(block, &view, sc)
...
func findBlock(t *testing.T, snapshot *compile.Snapshot, name string) *compile.Block {
	t.Helper()
	for i := range snapshot.Blocks {
		if snapshot.Blocks[i].Name == name {
			return &snapshot.Blocks[i]
		}
	}
	t.Fatalf("no block %s in the snapshot", name)
	return nil
}
