...
# <chart>/templates/crd-*.yaml by hand. Depends on manifests so the copy can
...
sync-helm-crds: manifests ## Regenerate the Helm CRD templates from config/crd/bases.
	...
	for f in config/crd/bases/*.yaml; do \
		...
		out="$$chart/templates/crd-$$kind.yaml"; \
		{ \
			echo '# Generated by "make sync-helm-crds" from config/crd/bases. DO NOT EDIT.'; \
			echo '# A satellite namespace of a composite runs no component: its filters'; \
			echo '# send checks to the baseline, and the CRD is cluster-scoped and already'; \
			echo '# installed by the baseline release.'; \
			echo '{{- if ne (include "ratelimit.mode" .) "satellite" }}'; \
			awk 'NR==1 && $$0 == "---" { next } \
	...
	echo "Synced $$(ls config/crd/bases/*.yaml | wc -l | tr -d ' ') Helm CRD template(s) into $$chart"; \
	done
...
helm-lint: ## Lint every chart against every resource profile, in every mode.
	...
	for profile in $$chart/resource-profiles/*.yaml; do \
		for mode in single baseline satellite; do \
			echo "--- lint $$chart with $$(basename "$$profile") as $$mode"; \
			case $$mode in \
				single) extra="" ;; \
...
.PHONY: promtool
promtool: $(PROMTOOL) ## Download promtool from the Prometheus release locally if necessary.
$(PROMTOOL): $(LOCALBIN)
	@[ -f "$(PROMTOOL)-$(PROMTOOL_VERSION)" ] && [ "$$(readlink -- "$(PROMTOOL)" 2>/dev/null)" = "$(PROMTOOL)-$(PROMTOOL_VERSION)" ] || { \
	set -e; \
	...
	mv "$(PROMTOOL)" "$(PROMTOOL)-$(PROMTOOL_VERSION)"; \
	} ;\
