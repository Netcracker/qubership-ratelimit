...
// +kubebuilder:rbac:groups=ratelimit.netcracker.com,namespace=ratelimit-system,resources=ratelimitpolicies/status,verbs=update
...
func (r *RateLimitPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	result, err := compile(ctx, r.Client, r.State, r.Namespace, r.limit())
	if err != nil {
		return ctrl.Result{}, err
	}
...
// limit is the size the namespace's configuration must fit: the configured
// one, or the ConfigMap's.
func (r *RateLimitPolicyReconciler) limit() int {
	if r.ConfigMapLimit > 0 {
		return r.ConfigMapLimit
	}
