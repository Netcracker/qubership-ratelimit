...
// portOf is the port to ask the endpoints of a slice on: the one the slice
// publishes under the metrics port name, which is the contract, since the
// operator knows nothing of the service's bind address. A slice can carry the
// port under another name or not at all, and then it has no port this probe
// can use.
